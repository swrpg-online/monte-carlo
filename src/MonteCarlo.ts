...
export class MonteCarlo {
   ...
   */
  public simulateAdversary(adversaryRating: number): MonteCarloResult {
    ...
    const sim = new MonteCarlo(config, this.iterations, false);
    return sim.simulate();
  }
   ...
   */
  public simulateAimedAttack(aimCount: number): MonteCarloResult {
    const config: SimulationConfig = {
      dicePool: this.dicePool,
      iterations: this.iterations,
      playerModifiers: {
   ...
   */
  public simulateSuperiorWeapon(superiorRating: number = 1): MonteCarloResult {
    const config: SimulationConfig = {
      dicePool: this.dicePool,
      iterations: this.iterations,
      playerModifiers: {
   ...
   */
  public simulateWithModifiers(modifiers: ModifierConfig): MonteCarloResult {
    const config: SimulationConfig = {
      dicePool: this.dicePool,
      iterations: this.iterations,
      modifiers,
   ...
   */
  public compareWithAndWithoutModifiers(modifiers: ModifierConfig): {
    ...
    // Run base simulation
    const baseSim = new MonteCarlo(this.dicePool, this.iterations, false);
    const base = baseSim.simulate();

    // Run modified simulation
    const modifiedSim = new MonteCarlo(
      {
        dicePool: this.dicePool,
        iterations: this.iterations,
        modifiers,
    ...
    const improvement = {
      successProbabilityDelta:
        modified.successProbability - base.successProbability,
      averageSuccessesDelta:
        modified.averages.successes - base.averages.successes,
      averageAdvantagesDelta:
        modified.averages.advantages - base.averages.advantages,
      criticalSuccessProbabilityDelta:
        modified.criticalSuccessProbability - base.criticalSuccessProbability,
    };
   ...
   * @returns MonteCarloResult with all modifiers applied
   */
  public simulateCombatScenario(
    sharpshooterRanks: number = 0,
    weaponSuperior: boolean = false,
    adversaryRating: number = 0,
  ): MonteCarloResult {
    const config: SimulationConfig = {
      dicePool: this.dicePool,
      iterations: this.iterations,
