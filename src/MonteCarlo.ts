...
export interface HistogramData {
  ...
  failures: { [key: number]: number };
  threats: { [key: number]: number };
...
export class MonteCarlo {
  ...
  private updateHistogram(result: DiceResult): void {
    ...
    this.histogram.failures[result.failures] =
      (this.histogram.failures[result.failures] || 0) + 1;
    this.histogram.threats[result.threats] =
      (this.histogram.threats[result.threats] || 0) + 1;
